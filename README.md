...
### POST `/invite`
- Purpose: Validate and assign a static invite code from Excel to a specific `newUser` address. Codes are pre-generated and stored in Microsoft Excel/OneDrive. Can be called directly from frontend. This is the first step - marks the invite code as used.
- Body params (JSON):
...
- Behavior:
  ...
  - **Note**: Referrer field is empty at this stage - it will be set when `/referral/redeem` is called
- Errors:
...
- Body params (JSON):
  - `code` (string): referral code from Neo4j (e.g. `ASV-3F9K2`) - generated by user A via `/referral/code`
  - `inviteCode` (string): invite code from Excel (e.g. `00d697e665b1667a`) - will be validated and marked as used automatically if not already used
  - `newUser` (string, 0x-address): wallet of the user who is joining with these codes
- Behavior:
  - **Step 1**: Validates invite code exists in Excel
    - If not used: marks it as used (sets `usedBy` = `newUser`, `usedAt` = timestamp)
    - If already used by this user: proceeds
    - If used by another user: rejects with error
    - If `assignedTo` is set, validates it matches `newUser`
  - **Step 2**: Looks up referral code in Neo4j to find the referrer (user A who generated the referral code)
  - **Step 3**: Validates referrer has allowed this new user (allow-list check via `/referral/allow`)
  - **Step 4**: Checks if referral relationship already exists (prevents duplicate referrals)
  - **Step 5**: Creates referral relationship in Neo4j (`REFERRED` relationship)
  - **Step 6**: Creates engagement records for both referrer and referred user
  - **Step 7**: Credits both users via engagement pipeline
  - **Step 8**: Updates Excel row with:
    - `usedBy`: newUser
    - `usedAt`: redemption timestamp
    - `referrer`: referrer address from referral code (user A's address)
    - `notes`: redemption details appended
- Errors:
  - `400 referral_code_required`: Referral code parameter missing
  - `400 invite_code_required`: Invite code parameter missing
  - `400 invalid_invite_code_not_found_in_excel`: Invite code doesn't exist in Excel
  - `400 invite_code_already_used_by_another_user`: Invite code was already used by a different user
  - `403 invite_code_not_assigned_to_this_user`: Invite code is assigned to a different address (if `assignedTo` is set)
  - `400 invalid_referral_code_not_found`: Referral code doesn't exist in Neo4j
  - `400 referral_already_exists`: Referral relationship already exists
  - `400 referral_not_allowed_by_referrer`: Referrer hasn't allow-listed this user
  - `400 self_referral_not_allowed`: User cannot refer themselves
- **Flow Options**:
  - **Option 1**: User has ONLY invite code → call `/invite` endpoint
  - **Option 2**: User has BOTH invite code AND referral code → call `/referral/redeem` directly (invite code will be validated automatically)
- Response:
  - On success: 
    ```json
    {
